...
async function generateContentWithFallback(params: {
  ...
  try {
    ...
    const isTransient = errMsg.includes("503") || errMsg.includes("unavailable") || errMsg.includes("busy") || errMsg.includes("overloaded");
...
async function configureServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[InkFlow AI] Server fully loaded on host 0.0.0.0, running on port ${PORT}`);
  });
}
